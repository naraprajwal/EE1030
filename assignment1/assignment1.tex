...
\begin{enumerate}
	...
	\item Let $f(x)$ be a function satisfying the condition $f(-x)=f(x)$ for real $x$.If $\prime{f(0)}$ exists, find its  value.\hfill (1987 - 2 Marks)
...
\item A function $f : R \rightarrow R$ satisfies the equation $f(x+y) = f(x)f(y)$ for all $x,y$ in $R$ and $f(x) \neq 0 $ for any $x$ in $R$. Let the function be differentiable at $x = 0$ and $f'(0)=2.$ Show that $\prime{f(x)} = 2f(x)$ for all $x$ in $R$. Hence, determine $f(x)$.\hfil (1990 - 4 Marks)
