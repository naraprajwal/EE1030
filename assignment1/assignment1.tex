...
\item Let $f(x) =$
$\begin{cases}
    \{1 + |\sin{x}|\}^\frac{a}{|\sin{x}|} & \text{if}\ - \frac{\pi}{6} < x < 0 \\
    b & \text{if}\ x = 0\\
    e^\frac{\tan{2x}}{\tan{3x}} & \text{if}\ 0 < x < \frac{\pi}{6}\\
    
\end{cases}$\\
Determine $a$ and $b$ such that $f(x)$ is continuous at $x=0$\hfill (1994 - 4 Marks)
\item  Let $f\brak{\frac{x+y}{2}} = \frac{f(x)+f(y)}{2}$ for real $x$ and $y$. If  $f'(0)$ exists and equals -1 and $f(0)=1$, find \\ 
...
\item $f'(0) = \lim_{n \to \infty} nf(\frac{1}{n})$ and $f(0) = 0$. Using this find\\
	$\lim_{x \to \infty}\ \brak{(n+1)\frac{2}{\pi}\cos^{-1}(\frac{1}{n})-n}\ , |\cos^{-1}\frac{1}{n}|<\frac{\pi}{2}$\\  
\hfill (2004 - 2 Marks)
